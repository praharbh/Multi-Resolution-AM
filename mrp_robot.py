...
class Kinematics():
	...
	def IK(robot_model, frame, seed = []):
		...
		# C:\ProgramData\Anaconda3\envs\pythonocc\Lib\site-packages\ikpy
		...
		# 	print("Low Error: [" + str(distance) + "," + str(alpha) + "," +\
		# 	 str(beta) + "," + str(gamma) + "]")
			return joint_var
	...
	def reverse_tool_transform(points):
		...
		for point in points:
			...
			gp_Vec(gp_Pnt(),P))
			# new_point_trsf = point_trsf.Multiplied(tool_trsf)
			...
			# new_point.append(point[0].Translated(gp_Pnt(),P))
			# vz = point[3].Reversed()
			# vx = point[1].Reversed()
			# vy = vz.Crossed(vx)
			# vx = vy.Crossed(vz)
			# vy = vz.Crossed(vx)
			# new_point.append(vx.Normalized())
			# new_point.append(vy.Normalized())
			# new_point.append(vz.Normalized())
